...
{
    public class DeadXPS
    {
        string WSXps;
        string Name;

        public DeadXPS(string filename)
        {
            FileInfo info = new FileInfo(filename);
            this.Name = info.Name;
            this.WSXps = Path.Combine(info.Directory.FullName, info.Name + "_WS.xps");
        }

        public void Print(List<Dead> list)
        {
            ...
            {
                PageContent pc = CreateWSPage(onepage, p, cs_doc);
                cs_doc.Pages.Add(pc);
                p++;
            } //end of all cs print
        ...
        public PageContent CreateWSPage(IEnumerable<Dead> onepagerec, int page, FixedDocument doc)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                var dead = (i < list.Count()) ? list[i] : new Dead();
                BlockWS block = PrintHelper.PrintWSBlock(dead.DeadName, dead.Relation + dead.LiveName, fp, i);
